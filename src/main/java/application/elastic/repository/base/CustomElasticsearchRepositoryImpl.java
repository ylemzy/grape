package application.elastic.repository.base;
...
public class CustomElasticsearchRepositoryImpl<T> extends AbstractElasticsearchRepository<T, String>
    ...
    /*@Override
    public Response perfomeRequest(RestClient restClient, RestRequest.Method method, Map<String, String> params, String query) throws IOException {
        ElasticsearchPersistentEntity persistentEntity = elasticsearchOperations.getPersistentEntityFor(this.getEntityClass());
        String queryFrom = "/" + persistentEntity.getIndexName() + "/" + persistentEntity.getIndexType() + "/_search";
        return restClient.performRequest(method.toString(), queryFrom, params, entity(query));
    }

    private HttpEntity entity(String query){
